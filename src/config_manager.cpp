...
bool ConfigManager::load() {
    ...
    bool result = parseJsonFile(content);
    if (result) {
        result = loadIKEv2Config();
    }
    return result;
}
...
bool ConfigManager::parseJsonFile(const std::string& content) {
    ...
    try {
        // Parse IKEv2 config file path
        size_t pos = content.find("\"ikev2_config_file\":");
        if (pos != std::string::npos) {
            size_t start = content.find("\"", pos + 20) + 1;
            size_t end = content.find("\"", start);
            if (start != std::string::npos && end != std::string::npos) {
                ikev2_config_file_ = content.substr(start, end - start);
            }
        }
...
void ConfigManager::setDefaults() {
    // IKEv2 config file default
    ikev2_config_file_ = "ikev2_client.conf";
...
bool ConfigManager::loadIKEv2Config() {
    std::ifstream file(ikev2_config_file_);
    if (!file.is_open()) {
        std::cerr << "Error: Cannot open IKEv2 config file: " << ikev2_config_file_ << std::endl;
        std::cerr << "Using default IKEv2 configuration values" << std::endl;
        return true; // Use defaults
    }
    ...
    try {
        // Parse IKEv2 configuration from separate file
        size_t pos = content.find("\"local_id\":");
        if (pos != std::string::npos) {
        ...
        std::cout << "IKEv2 configuration loaded from: " << ikev2_config_file_ << std::endl;
        return true;

    } catch (const std::exception& e) {
        std::cerr << "Error parsing IKEv2 config file: " << e.what() << std::endl;
        std::cerr << "Using default IKEv2 configuration values" << std::endl;
        return true; // Continue with defaults
    }
}
